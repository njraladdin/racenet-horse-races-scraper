...
const BASE_JSON_OUTPUT_DIR = path.join(DATA_DIR, 'output_forms_json_historical');
const BASE_CSV_OUTPUT_DIR = path.join(DATA_DIR, 'output_forms_csv_historical');
...
async function extractHistoricalFormsData() {
  try {
    ...
    logger.info(`Extracting historical forms data for ${dates.length} days from ${dates[0]} to ${dates[dates.length - 1]}`);
          ...
          try {
            ...
            if (results) {
              ...
              if (getConfig('SAVE_RAW_DATA')) {
                const rawOutputFile = path.join(jsonOutputFolder, `${slug}_raw`);
    ...
    logger.success(`Completed extracting historical forms data for all dates in ${totalTimeMin.toFixed(2)} minutes`);
  } catch (error) {
...
if (require.main === module) {
  ...
  extractHistoricalFormsData().catch(error => {
    console.error('Failed to extract historical forms data:', error);
    process.exit(1);
  });
}

module.exports = { extractHistoricalFormsData }; 